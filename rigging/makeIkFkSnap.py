...
def l_ikFkSnap():
    ...
    if mc.getAttr('l_armA_Attr_CON.IkFkSnap'):
        ...
        mc.xform("l_armA_Pv_CON", t=[mc.xform("l_armB_Fk_CON", t=1, q=1, ws=1)[i] + arm01Vec[i] * .75 + arm02Vec[i] * .75 for i in range(3)], ws=1)
        mc.setKeyframe("l_armA_Pv_CON")
...
mc.scriptJob(attributeChange=['l_armA_Attr_CON.IkFkSnap',l_ikFkSnap])
'''
...
def r_ikFkSnap():
    ...
    if mc.getAttr('r_armA_Attr_CON.IkFkSnap'):
        ...
        # Transform polevector
        mc.xform("r_armA_Pv_CON", t=[mc.xform("r_armB_Fk_CON", t=1, q=1, ws=1)[i] + arm01Vec[i] * .75 + arm02Vec[i] * .75 for i in range(3)], ws=1)
        mc.setKeyframe("r_armA_Pv_CON")
...
mc.scriptJob(attributeChange=['r_armA_Attr_CON.IkFkSnap',r_ikFkSnap])
'''
